...
#In the Unrolled and d-flow methods, change the originally method="midpoint" to method=args.ode_solver.
# Adjoint method uses the same forward_solver to find the forward x_hat (in the forward pass), keeping the original handwritten Euler reverse unchanged.
# New command-line parameter --ode_solver, default "midpoint"
...
def optimise_z_unrolled(physics, y, x_gt, wrapped, iter_max, device, z, solver_name):
...
def optimise_z_dflow(physics, y, x_gt, wrapped, iter_max, device, z, solver_name):
    solver = ODESolver(velocity_model=wrapped)
    ...
    for i in range(iter_max):
        def closure():
            ...
            x_hat = solver.sample(
                time_grid=t_grid,
                x_init=z,
                method=solver_name, step_size=1/STEPS,
                enable_grad=True)
            ypred = physics(x_hat).squeeze(0).squeeze(0).transpose(0,1)
            loss  = ((ypred - y)**2).sum() + 0.01*(z**2).mean()
            loss.backward()
...
def optimise_z_adjoint(physics, y, x_gt, wrapped, iter_max, device, z, solver_name):
    solver = ODESolver(velocity_model=wrapped)
    opt    = torch.optim.Adam([z], lr=1e-1)
    losses, psnrs, z_norms = [], [], []
    for i in range(iter_max):
...
def run_once(seed, method, init, cfg_path, weight_path, iter_max, device, output_dir, ode_solver):
    ...
    if method=="unrolled":
        ...
        rec, losses, psnrs, z_norms = optimise_z_unrolled(physics, y, x_gt, wrapped, iter_max, device, z0, ode_solver)
        if device=="cuda":
        ...
        if device=="cuda":
           torch.cuda.reset_peak_memory_stats()
        rec, losses, psnrs, z_norms = optimise_z_dflow(physics, y, x_gt, wrapped, iter_max, device, z0, ode_solver)
        if device=="cuda":
           peak = torch.cuda.max_memory_allocated()/(1024**3)
        ...
        if device=="cuda":
            torch.cuda.reset_peak_memory_stats()
        rec, losses, psnrs, z_norms = optimise_z_adjoint(physics, y, x_gt, wrapped, iter_max, device, z0, ode_solver)
        if device=="cuda":
            peak = torch.cuda.max_memory_allocated()/(1024**3)
...
def main():
    ...
    p.add_argument("--ode_solver",  type=str, default="midpoint",
                   help="Forward solver: midpoint|euler|rk4|dopri5")
