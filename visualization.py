"""
Visualize the unconditional flow-based sampling process from pure noise to image using a trained flow-matching model.
Generates intermediate images and a grid of the entire sampling trajectory.
"""
import os
import argparse
import yaml
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize flow-based sampling process"
    )
    parser.add_argument(
        "--cfg_path", type=str, required=True,
        help="Path to the flow-matching model YAML config file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sampler_steps", type=int, default=50,
        help="Number of ODE sampling steps (time grid length)"
    )
    parser.add_argument(
        "--img_size", type=int, default=128,
        help="Height and width of the square images"
    )
    parser.add_argument(
        "--output_dir", type=str, default="visualization",
        help="Directory to save intermediate images and grid"
    )
    parser.add_argument(
    ...
    # Create and save grid of all steps
    # Arrange in square grid if possible
