...
#     # 3. latent z + adjoint optimization
#     z = torch.randn_like(x, requires_grad=True, device=device)
#     opt = torch.optim.Adam([z], lr=1e-1)

#     losses = []
#     for i in tqdm(range(args.iter_max), desc="Adjoint optim"):
#         opt.zero_grad()
...
#             model_out, jvp = torch.autograd.functional.jvp(
#                 lambda v: wrapped(v, t_val),
#                 (xt,),
#                 (lam,)
#             )
#             xt = xt - dt*model_out
...
#             gt_np = np.clip(gt_np, 0, 1) #  Manually clip the original image to prevent image inversion
#             out = np.concatenate([gt_np, xh_np], axis=1) # Spliced and saved images
#             plt.imsave(f"img_adj_{i}.png", out, cmap="gray")
