# Monitoring Latent Variable Norm ‖z‖ :
#Record the norm (L2) of z (i.e., x(0)) for each N iterations to analyze whether there is overfitting
# In each optimise_* function, add z_norms = [] and append z_norms.append(torch.norm(z).item()) after each iteration.
#fter training, draw and save the z_norms curve to z_norm.png
import os
import argparse
import random
import yaml
...
def optimise_z_unrolled(physics, y, x_gt, wrapped, iter_max, device, z):
    ...
    for i in range(iter_max):
        ...
        z_norms.append(torch.norm(z).item()) #record the L2 norm of z
...
def run_once(seed, method, init, cfg_path, weight_path, iter_max, device, output_dir):
    ...
    if method=="unrolled":
        ...
        if device=="cuda":
            torch.cuda.reset_peak_memory_stats()
        rec, losses, psnrs, z_norms = optimise_z_adjoint(physics, y, x_gt, wrapped, iter_max, device, z0)
        if device=="cuda":
            peak = torch.cuda.max_memory_allocated()/(1024**3)
