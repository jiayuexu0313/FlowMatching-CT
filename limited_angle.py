...
def build_physics(device, max_angle_deg, num_angles):
    max_angle = max_angle_deg
    angles = torch.linspace(0, max_angle, num_angles, device=device, dtype=torch.float32)
    return Tomography(
        img_shape=(1,128,128),
        angles=angles,
        geometry="parallel",
...
def get_data(device, max_angle_deg, num_angles):
    physics = build_physics(device, max_angle_deg, num_angles)
...
def run_once(seed, method, init, cfg_path, weight_path, iter_max, device, output_dir, max_angle_deg, num_angles):
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed)
    if device=="cuda": torch.cuda.manual_seed_all(seed)

    physics, y, x_gt = get_data(device, max_angle_deg, num_angles)

    detector_resolution = np.sqrt(128**2 + 128**2)
    undersampling = (num_angles * detector_resolution) / (128*128)
    print(f"[seed={seed}] max_angle_deg={max_angle_deg},num_angles={num_angles}, undersampling={undersampling:.3f}")
    ...
    od_seed = os.path.join(output_dir,
                           f"{method}_{init}_max{int(max_angle_deg)}_num{num_angles}",
                           f"seed{seed}")
    ...
    od_seed = os.path.join(output_dir, f"{method}_{init}_max{int(max_angle_deg)}_num{num_angles}", f"seed{seed}")
    os.makedirs(od_seed, exist_ok=True)
...
def main():
    ...
    p.add_argument("--max_angle_deg", type=float, default=90.0, help="The maximum collection Angle range (in degrees), such as 180, 90, and 60")
    p.add_argument("--num_angles", type=int, default=60, help="The values are the same as max_angle_deg, 90 and 60")
